    ...
    if ($_SERVER["REQUEST_METHOD"] != "POST") 
        ...
        try {
            if (!isset($_POST["submit"]) || !isset($_POST["reposName"]) || empty($_POST["reposName"]) || !isset($_FILES["fileUpload"]["type"]) || empty($_FILES["fileUpload"]["type"])){
                error_log("\n" . "One or more fields are empty in addNewRepo.php", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
            ...
            if (($fileType != "application/zip") && ($fileType != "application/x-zip-compressed")) {
                error_log("\n" . "Uploaded file is not a .zip file, contenuto di filetype: $fileType, contenuto di FILES: " . $_FILES["fileUpload"]["type"], 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
            ...
            if (is_dir("/SAW/SAWFinalProject/repos/$email/$repoName")) {
                error_log("\n" . "A repo with this name already exists, please chooese another name", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("A repo with this name already exists, please chooese another name");
            }

            if (preg_match("/[.,\/]/", $repoName)) {
                error_log("\n" . "$email tried to create a repo with a name that contained invalid characters", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
            ...
            if (preg_match("/[.,\/]/", $fileNameWithoutExtension)) {
                error_log("\n" . "$email tried to upload a file with a name that contained invalid characters", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");

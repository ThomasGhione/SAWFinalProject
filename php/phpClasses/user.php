    ...
    class User {
        ...
        function __construct (bool $login) {            
            ...
            try {
                ...
                if (empty($_POST["email"]) || empty($_POST["pass"])) {
                    error_log("\n" . "Empty parameters have been passed to the form", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                ...
                if ($email === false || strlen($email) > 64) {
                    error_log("\n" . "Invalid email", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
        ...
        private function cRegister(string $email, string &$password): void {
            try {
                ...
                if (strlen($firstname) > 64) {
                    error_log("\n" . "First name is too long", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                    throw new Exception("First name is too long. Please try again");
                }       
                if (strlen($lastname) > 64) {
                    error_log("\n" . "Last name is too long", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                ...
                if ($this->isPasswordWeak($password)) {
                    error_log("\n" . "Password isn't strong enough", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                ...
                if (!$this->passwordMatches($password, $confirm)) {
                    error_log("\n" . "Passwords don't match", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
        ...
        private function isPasswordWeak(string &$password): bool {
            if (strlen($password) < 8) {
                error_log("\n" . "Pw is too short", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("Password is too short, please choose a longer password");
            }
            if (strlen($password) > 24) {
                error_log("\n" . "Pw is too long", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("Password is too long, please choose a shorter password");
            }
            if (!preg_match('@[A-Z]@', $password)) {
                error_log("\n" . "Pw doesn't contain uppercase letters", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("Password doesn't contain uppercase letters, please try again");
            }
            if (!preg_match('@[a-z]@', $password)) {
                error_log("\n" . "Pw doesn't contain lowercase letters", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("Password doesn't contain lowercase letters, please try again");
            }
            if (!preg_match('@[0-9]@', $password)) {
                error_log("\n" . "Pw doesn't contain numbers", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
                throw new Exception("Password doesn't contain numbers, please try again");
            }
            if (!preg_match('@[^\w]@', $password)) {
                error_log("\n" . "Pw doesn't contain special characters", 3, $_SERVER["DOCUMENT_ROOT"] . "/SAW/SAWFinalProject/texts/errorLog.txt");
